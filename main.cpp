...
{
	Mat image1 = imread(string(argv[1]), CV_LOAD_IMAGE_COLOR);
	Mat image2 = imread(string(argv[2]), CV_LOAD_IMAGE_COLOR);

	// resize smaller image if they do not match
	if (image1.size().width != image2.size().width || image1.size().height != image2.size().height)
	{
		uint64_t image1Size = image1.size().width * image1.size().height;
		uint64_t image2Size = image2.size().width * image2.size().height;
